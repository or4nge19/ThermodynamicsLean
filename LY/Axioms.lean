...
/-- Inhabited instance for the 0-scaled system, via the ZSystem identification. -/
instance instInhabitedScaleZeroState (System : Type u) [TW : ThermoWorld System] {Γ : System} :
    Inhabited (TW.State (TW.scale 0 Γ)) :=
  ⟨
    (Equiv.cast
      (congrArg TW.State (TW.scale_zero_is_ZSystem Γ))).symm
...
/-- If the component state types are inhabited, so is the composed state type. -/
instance instInhabitedCompState (System : Type u) [TW : ThermoWorld System]
    {Γ₁ Γ₂ : System} [Inhabited (TW.State Γ₁)] [Inhabited (TW.State Γ₂)] :
    Inhabited (TW.State (TW.comp Γ₁ Γ₂)) :=
  ⟨comp_state ((default : TW.State Γ₁), (default : TW.State Γ₂))⟩

-- A convenience simp: the chosen default of 0•Γ is exactly the casted default of ZSystem.
@[simp] lemma default_scale_zero_cast (System : Type u) [TW : ThermoWorld System] {Γ : System} :
  (default : TW.State (TW.scale 0 Γ)) =
    (Equiv.cast (congrArg TW.State (TW.scale_zero_is_ZSystem Γ))).symm
      (default : TW.State TW.ZSystem) := rfl
...
open InnerProductSpace RealInnerProductSpace Convex
...
/-- The unit vector in the direction of the energy coordinate in `SimpleSystemSpace n`.
    This corresponds to the first basis vector in `ℝⁿ⁺¹`. -/
def energy_basis_vector (n : ℕ) : SimpleSystemSpace n :=
...
class SimpleSystemFamily (n : ℕ) (is_in_family : System → Prop) where
  ...
  /-- **S2 (Unique Tangent Plane & Lipschitz Pressure)**:
      For each state `X`, the normal cone to the forward sector `A_X` at `X`'s
      coordinates is a single ray generated by a non-zero vector `u(X)`.
      This generator `u(X)` must not be orthogonal to the energy axis, and the
      pressure `P(X)`, derived from `u(X)`, must be locally Lipschitz. -/
  S2_NormalCone_is_Ray {Γ} (h_in : is_in_family Γ) (X : TW.State Γ) :
    let ss := get_ss_inst Γ h_in
    let C_X := Set.image (fun Y => (ss.state_equiv Y).val) (ForwardSector X)
    let x_coord := (ss.state_equiv X).val
    -- 1. The normal cone is a ray generated by some `u`.
    ∃ u : SimpleSystemSpace n, u ≠ 0 ∧
      (normalConeAt C_X x_coord : Set (SimpleSystemSpace n)) = {c • u | c ≥ 0} ∧
    -- 2. The generator has a non-zero energy component ("finite slope").
      ⟪u, energy_basis_vector n⟫_ℝ ≠ 0

  /-- **S2 (Lipschitz Pressure)**: The function that maps a state `X` to the
      pressure vector `P(X)` is locally Lipschitz with respect to the coordinates.
      The pressure `P(X)` is derived from the generator of the normal cone. -/
  S2_Lipschitz_Pressure {Γ} (h_in : is_in_family Γ) : ∃ P_coord : SimpleSystemSpace n → EuclideanSpace ℝ (Fin n),
    -- P_coord is defined on the ambient simple-system space (not on the Subtype), so pass it directly.
    LocallyLipschitzOn (↑(get_ss_inst Γ h_in).space) P_coord ∧
    ∀ x : (get_ss_inst Γ h_in).space,
      let X : TW.State Γ := (get_ss_inst Γ h_in).state_equiv.symm x
      let u := Classical.choose (S2_NormalCone_is_Ray h_in X)
      let u_E : ℝ := ⟪u, energy_basis_vector n⟫_ℝ
      let u_W_proj := (work_subspace : Subspace ℝ (SimpleSystemSpace n)).starProjection u
      -- Convert the work projection (an element of SimpleSystemSpace n = Fin (n+1) → ℝ)
      -- to EuclideanSpace ℝ (Fin n) by dropping the energy (first) coordinate.
      let u_W : EuclideanSpace ℝ (Fin n) := fun i => u_W_proj (Fin.succ i)
          -- NOTE on sign convention: The LY tangent plane is U - U₀ + P•(V - V₀) = 0, with normal (1, P).
    -- The forward sector is above this plane. A standard outer normal `u` to the forward sector
    -- would be proportional to -(1, P). This would give u_E = -k and u_W = -kP, so P = (1/u_E)u_W.
    -- The sign chosen here may reflect a different convention and needs careful verification.
      P_coord x.val = (1 / u_E) • u_W
...
/-- A non-computable definition to extract the pressure function `P` whose existence
is guaranteed by `S2_Lipschitz_Pressure`. -/
noncomputable def pressureOf {Γ : System} (h_in : is_in_family Γ)
    [ssf : SimpleSystemFamily n is_in_family] :
    SimpleSystemSpace n → EuclideanSpace ℝ (Fin n) :=
  Classical.choose (ssf.S2_Lipschitz_Pressure h_in)

omit ssf in
/-- The pressure function `pressureOf` is locally Lipschitz on the state space. -/
theorem pressureOf_locallyLipschitzOn {Γ : System} (h_in : is_in_family Γ)
    [ssf : SimpleSystemFamily n is_in_family] :
    LocallyLipschitzOn (↑(ssf.get_ss_inst Γ h_in).space) (pressureOf h_in) :=
  (Classical.choose_spec (ssf.S2_Lipschitz_Pressure h_in)).1

omit ssf in
/-- The pressure function `pressureOf` satisfies the geometric derivation for every state. -/
theorem pressureOf_eq_geometric_derivation {Γ : System} (h_in : is_in_family Γ)
    [ssf : SimpleSystemFamily n is_in_family] (x : (ssf.get_ss_inst Γ h_in).space) :
    pressureOf h_in x = (Classical.choose (ssf.S2_Lipschitz_Pressure h_in)) x := by
  rfl

/-
/--
S2_Coherence: adiabatically equivalent states have equal pressure.
It can be proven as a theorem after establishing the reversibility on the boundary (LY Thm 3.7)
-/
theorem S2_Coherence_from_axioms (Γ : System) (h_in : is_in_family Γ) {X Y : TW.State Γ}
    (h_equiv : X ≈ Y) :
    pressureOf h_in ((ssf.get_ss_inst Γ h_in).state_equiv X).val =
    pressureOf h_in ((ssf.get_ss_inst Γ h_in).state_equiv Y).val := by
  sorry
